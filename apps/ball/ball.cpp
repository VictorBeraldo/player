...
void setup()
{
  ...
  // Set palette 2 (shadow) to all gray
  int i;
  for (i = 0; i < 256; i++)
    GD.wr16(RAM_SPRPAL + (2 * (512 + i)), RGB(64, 64, 64));

  // Set color 255 is transparent in all three palettes
  GD.wr16(RAM_SPRPAL + 2 * 0xff, 0x8000);
  GD.wr16(RAM_SPRPAL + 2 * 0x1ff, 0x8000);
  GD.wr16(RAM_SPRPAL + 2 * 0x2ff, 0x8000);

  GD.wr16(BG_COLOR, RGB(127, 127, 127));
}

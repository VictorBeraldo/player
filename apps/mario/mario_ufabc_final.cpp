...
void loop() {
	...
	// -----------------------------> Pulo <-----------------------------
	// Mario
	if ((control & CONTROL_SPACE) && !mario_pulando) {
		mario_pulando = true;
		t0_pulo_m = millis();
		y0_pulo_m = ymario;
	}
	...
	// Luigi
	if ((control & CONTROL_SPACE) && !luigi_pulando) {
		luigi_pulando = true;
		t0_pulo_l = millis();
		y0_pulo_l = yluigi;
	}
	...
	// -----------------------------> Animacao <-----------------------------
	// Mario
	if (!(control & CONTROL_RIGHT) && !(control & CONTROL_LEFT)) {
		m_animando = false;
	}
	if (m_animando) {
		if (anim_delay == 1) {
...
// Intervalo de Sprites
// 0-9 - Tartarugas
// 10-19 - Nabos
// 20-29 - Provas (chuva de nabos)
...
{
  GD.copy(RAM_SPRIMG, lena_img, sizeof(lena_img));
  GD.copy(RAM_SPRPAL, lena_pal, sizeof(lena_pal));
  int i;
  for (i = 0; i < 64; i++)
...
int test_coproc()
{
  ...
  for (regime = 0; regime < 6; regime++) {
	...
	/* <<< THIS TEST FAILS IN EMULATOR >>>
	if (cps < 1000000)
      FAIL;
	*/

	printf("cps %u / 1000000\n", (unsigned int)cps);
